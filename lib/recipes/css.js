  ...
  rewritePaths: function(wake, source, pathname, settings) {
    ...
    var css = source.data.replace(URL_G, function(match) {
      var url = match.match(URL),
          pth = self.resolve(url[2], source.path, settings.root);

      if (!pth) return match;

      var target = wake.target(pth).targetClosestTo(pathname);
      pth = path.relative(path.dirname(pathname), target.unhashify());
