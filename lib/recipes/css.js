...
var CSS = {
  ...
  expand: function(wake, source, settings, paths) {
    ...
    css = css.replace(IMPORT, function(match, url, string) {
      ...
      if (!url) return match;
      imports.push(url.replace(QUERY, ''));
      return '';
    });
    ...
    (css.match(URL_G) || []).forEach(function(url) {
      ...
      if (!pathname) return;
      urls.push(pathname.replace(QUERY, ''));
    });
  ...
  rewritePaths: function(wake, source, pathname, settings) {
    ...
    var css = source.data.replace(URL_G, function(match) {
      var url = match.match(URL),
          ...
          query, target, targetPath, hosts, host;

      if (!pth) return match;

      query = pth.match(QUERY);
      query = query ? query[0] : '';
      ...
      if (settings.inline) {
        ...
        pth   = host.replace(/\/*$/, '/') + path.relative(settings.targetRoot, targetPath) + query;
      } else {
        pth = path.relative(path.dirname(pathname), targetPath) + query;
      }
