...
var CSS = {
  ...
  generate: function(wake, pathname, sources, settings) {
    ...
    return wake.target(pathname, sources, function(deps) {
      ...
      if (settings.minify !== false) css = csso.justDoIt(css);
  ...
  rewritePaths: function(wake, source, pathname, settings) {
    ...
    var css = source.data.replace(URL_G, function(match) {
      ...
      target = wake.target(pth).targetClosestTo(pathname).unhashify();

      if (settings.hosts) {
        if (!settings.targetRoot) throw new Error('No targetRoot specified to create absolute URL for "' + pth + '" referenced by "' + source.path + '"');
        host = settings.hosts[parseInt(digest.hash(target).substr(0,8), 16) % settings.hosts.length];
        pth  = host.replace(/\/*$/, '/') + path.relative(settings.targetRoot, target);
      } else {
        pth = path.relative(path.dirname(pathname), target);
      }
