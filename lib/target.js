...
Target.prototype.addDependent = function(target) {
  this._deps.forEach(function(d) { d.addDependent(target) });
  if (typeof this.path !== 'number' || typeof target.path !== 'string') return;

  this.mtime = (this.mtime === undefined)
             ? target.mtime || 0
             : Math.min(this.mtime, target.mtime || 0);
};
